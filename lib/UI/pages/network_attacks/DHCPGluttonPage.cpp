...
DHCPGluttonPage::DHCPGluttonPage(GFXForms *_screen) {
  text = new Text(_screen, ST77XX_WHITE, english_words->at(RUNNING_DHCPGLUTTON_KEY));
  client_count = new Text(_screen, ST77XX_WHITE, english_words->at(CREATED_DHCPGLUTTON_CLIENTS_KEY) + String(0));
...
void DHCPGluttonPage::update_packet_count(int count) {
  client_count->set_text(english_words->at(CREATED_DHCPGLUTTON_CLIENTS_KEY) + String(count));
}
