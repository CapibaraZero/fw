...
void WifiAttack::sniff(int delay, bool inf, FS sd)
{
    WifiSniffer sniffer = WifiSniffer(PCAP_FILE(PCAP_TYPE).c_str(), sd);
    if (inf)
    {
        for (;;)
        {
...
void WifiAttack::sniff(int max_ch, int delay, bool inf, FS sd)
{
    WifiSniffer sniffer = WifiSniffer(PCAP_FILE(PCAP_TYPE).c_str(), sd);
    if (inf)
    {
...
void WifiAttack::sniff_channel(int channel, int time, bool inf, FS sd)
{
    WifiSniffer sniffer = WifiSniffer(PCAP_FILE(PCAP_TYPE).c_str(), sd);
    esp_wifi_set_channel(channel, (wifi_second_chan_t)NULL);
    if (inf)
    {
        for (;;)
        {
    ...
    {
        delay(time); // Wait before changing channel, sniffer will sniff meanwhile.
    }
}