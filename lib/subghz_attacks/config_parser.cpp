...
static void param_check(float value, const char *name, float initial_range, float end_range, float default_value, float *var) {
...
SubGHZRAWRecorderConfig parse_raw_record_config() {
    ...
    if(raw_record_config["bandwidth"].isNull()){
        ...
        param_check(raw_record_config_bandwidth, "Bandwidth", 58.03, 812.50, DEFAULT_BANDWIDTH, &params.bw);
    }

    if(raw_record_config["deviation"].isNull()){
        Serial.printf("Deviation is null. Using default deviation: %f\n", DEFAULT_DEVIATION);
    }else{
        param_check(raw_record_config_deviation, "Deviation", 1.58, 380.85, DEFAULT_DEVIATION, &params.deviation);
    }
    ...
    if(raw_record_config["rssi_threshold"].isNull())
        ...
        if(raw_record_config["rssi_threshold"] > 0)
            Serial.printf("RSSI threshold can't be positive. Using default value %f\n", DEFAULT_RSSI_THRESHOLD);
        else
