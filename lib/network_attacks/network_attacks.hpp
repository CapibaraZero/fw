...
private:
    ...
    /// @brief Create the AP from config or, if not provided, create the default AP
    void create_ap();
public:
    NetworkAttacks(/* args */);
    ~NetworkAttacks();
    void dhcp_starvation();
    uint8_t get_generated_clients() { return generated_clients; };	// Try to reduce a bit the overhead
