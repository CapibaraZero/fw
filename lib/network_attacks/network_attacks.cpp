...
void NetworkAttacks::create_ap() {
    ...
    if (error) {
        LOG_ERROR("Error reading captive portal config. Using default configuration");
        create_default_ap();
        return;
    }
...
void NetworkAttacks::dhcp_starvation()
{
    ...
    for (;;)
    {
        ...
        generated_clients++;
        delay(500);	// Reduce redundant connections(Sometimes ESP get same IP due to weak random)
    }
}
...
void NetworkAttacks::start_evilportal() {
    evilPortal->start_portal();
}
